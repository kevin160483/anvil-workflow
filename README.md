...
編輯按鈕預設是隱藏的。在網址後面加 `?edit=1` 開啟一次，這台裝置就會記住：

```
https://<你的帳號>.github.io/<repo>/?edit=1
```

之後直接開首頁右上角就有 **編輯**。要關掉換成 `?edit=0`。同事拿到一般網址看不到按鈕。

> 這只是把按鈕藏起來，不是權限控管——網頁是公開的靜態檔，會看原始碼的人找得到參數。
> 但那不影響安全：編輯器只在瀏覽器裡跑，任何人改了也只能下載到自己電腦，
> 要讓內容真的生效必須把 `data.json` 推回這個 repo，而那需要你的 GitHub 權限。
> 如果連按鈕都不想出現在公開網站上，就把 `index.html` 留一份在自己電腦上編輯，
> 公開版只放沒有編輯器的那份。
